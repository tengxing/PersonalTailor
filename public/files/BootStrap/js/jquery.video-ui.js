...
(function( $ ){
    ...
    $.fn.videoUI = function( options ) {  
        ...
        this.bind('click',function(){
            ...
            $(this).children('i').toggleClass('glyphicon-pause');
            $(this).toggleClass('pauseMedia');   
            return false;
        });
        
        if (settings.playMedia){
            $('.'+controllerClass).append('<a href="#" class="playMedia"><i class="glyphicon glyphicon-play"></i></a>');                        
            ...
            this.bind('play',function(){           
                $('.'+controllerClass+' .playMedia i').addClass('glyphicon-pause');
                $('.'+controllerClass+' .playMedia').addClass('pauseMedia');     
            });
            
            this.bind('pause',function(){           
                clearInterval(timer);
                $('.'+controllerClass+' .playMedia i').removeClass('glyphicon-pause');
        ...
        if (settings.volumeMedia){
            ...
            var on='';
            var html = '<i class="mute glyphicon glyphicon-volume-down"></i><ul class="volumeMedia list-unstyled">';
            for(i=0;i<10;i++){
            ...
            if (volume < 5 ) $('.'+controllerClass+ ' .mute').addClass('glyphicon-volume-down').removeClass('glyphicon-volume-up');
            else $('.'+controllerClass+ ' .mute').addClass('glyphicon-volume-up').removeClass('glyphicon-volume-down');
            ...
            $('.'+controllerClass+ ' .volumeMedia li').click(function(){
                ...
                if ($(this).index()+1 < 5 ) $('.'+controllerClass+ ' .mute').addClass('glyphicon-volume-down').removeClass('glyphicon-volume-up');
                else $('.'+controllerClass+ ' .mute').addClass('glyphicon-volume-up').removeClass('glyphicon-volume-down');
            });
            
            $('.'+controllerClass+ ' .mute').click(function(){                
                if (video.volume >0){   
                    $('.'+controllerClass+ ' .mute').addClass('glyphicon-volume-off').removeClass('glyphicon-volume-down').removeClass('glyphicon-volume-up');
                    $('.'+controllerClass+ ' .volumeMedia li').removeClass('on');
                    video.volume = 0; 
                }
            });
        ...
        if (settings.fullscreenMedia){
            $('.'+controllerClass).append('<i class="glyphicon glyphicon-fullscreen "></i>');
            $('.'+controllerClass+ ' .glyphicon-fullscreen').click(function(){
